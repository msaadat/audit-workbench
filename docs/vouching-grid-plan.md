...
`assurance_scope` is computed by the domain service and cannot be supplied or upgraded by the model, route caller, or frontend. It is reported, not enforced: the scope travels with the roll-up, the working paper, and the report context so a reader can see how the items were reached, but it does not gate the conclusion. Whether targeted evidence is enough to conclude on is the auditor's judgment to make and disclose, the same as an incomplete evidence base. The one structural rule left is that a test which has not run cannot carry a conclusion.
