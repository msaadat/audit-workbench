...
path.  
**Cutover:** clean — pre-consolidation workspaces are not migrated (section 8).
...
Renaming `analysis_workflow_v1` here changes the workflow definition hash before
its persistence target changes in WP4. That is safe under clean cutover — there
are no runs to keep dispatchable — and it satisfies section 7.1's requirement
that the new persistence target carry a new workflow identity.

**Gate:** the full backend suite and the frontend production build pass with no
behaviour change, and the following returns nothing:

```bash
rg -n "datatest:|data_tests|DataTests|DataTestResults|data-tests" backend/app frontend/src
```
