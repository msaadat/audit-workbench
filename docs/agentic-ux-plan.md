...
6. **The `file/` segment and the `?tab=` layer were both removed later (2026-08-29).** Work products sit directly under the workspace — `/workspace/:id/apm`, `/workspace/:id/coverage` — matched by a `:section` child that every static sibling outranks. `file/` grouped them while the audit file was a surface with its own rail; once the record became the index (§ *Engagement record*), the segment named nothing a reader could see. The same commit deleted the global `?tab=` `beforeEach` in item 4 and the `LEGACY_TABS` map behind it: pre-surface URLs no longer resolve, and `engagement_progress.py` now emits destination names (`apm`, `rcm`) rather than `planning` / `planning?view=rcm`, so `routeForTarget` validates a target instead of translating one. Reports keep generating `?tab=findings&finding=` links, which are parsed for citation status and never rendered as anchors — `MarkdownEditor` draws no links — so nothing in the app navigates by that query any more.

**Also worth knowing**

- `workspaceQuery`/`cleanWorkspaceQuery` are gone. Navigation is `useWorkspaceNav()` — `nav.replace('rcm', { rcm: id })` — with destination names unchanged, so the ~25 call sites changed shape, not intent. `nav.replaceTarget()` handles server-supplied `DashboardTarget`s, which speak destination names too as of deviation 6.
