...
def list_cycle_rulesets(workspace_id: str):
    ws = _ws(workspace_id)
...
def create_cycle_ruleset(workspace_id: str, payload: dict = Body(...)):
...
def get_cycle_ruleset(workspace_id: str, ruleset_id: str):
    ws = _ws(workspace_id)
    return _reviewable(ws, cycle_rulesets.get(ws, ruleset_id))


@router.patch("/cycle-rulesets/{ruleset_id}")
def patch_cycle_ruleset(workspace_id: str, ruleset_id: str, payload: dict = Body(...)):
...
def measure_cycle_ruleset(workspace_id: str, ruleset_id: str):
...
def approve_cycle_ruleset(workspace_id: str, ruleset_id: str, payload: dict = Body(default={})):
...
def reject_cycle_ruleset(workspace_id: str, ruleset_id: str):
    ws = _ws(workspace_id)
...
def get_document_tests_summary(workspace_id: str):
...
def get_document_test_meta(workspace_id: str):
...
def patch_document_test(workspace_id: str, test_id: str, payload: dict = Body(...)):
    return doc_tests.update_test(_ws(workspace_id), test_id, payload)


@router.delete("/doc-tests/{test_id}")
def delete_document_test(workspace_id: str, test_id: str):
    doc_tests.remove_test(_ws(workspace_id), test_id)
    return {"ok": True}


@router.post("/doc-tests/{test_id}/items/{item_id}/documents")
def attach_document(workspace_id: str, test_id: str, item_id: str, payload: dict = Body(...)):
    ws = _ws(workspace_id)
...
def detach_document(workspace_id: str, test_id: str, item_id: str, document_id: str):
...
def patch_comparisons(workspace_id: str, test_id: str, item_id: str, payload: dict = Body(...)):
...
def patch_document_test_item(workspace_id: str, test_id: str, item_id: str, payload: dict = Body(...)):
    return doc_tests.update_item(_ws(workspace_id), test_id, item_id, payload)


@router.post("/doc-tests/dispositions")
def post_document_test_dispositions(workspace_id: str, payload: dict = Body(...)):
...
def draft_procedure_results(workspace_id: str, procedure_id: str):
    raise WorkspaceError(
...
def get_working_paper(workspace_id: str, procedure_id: str):
    return working_papers.render(_ws(workspace_id), procedure_id)


@router.post("/matching/compare")
def compare_values(payload: dict = Body(...)):
...
def list_evidence_requests(workspace_id: str):
