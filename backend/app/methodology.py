...
def search(workspace: Workspace, query: str, *, limit: int = 10) -> list[dict]:
    ...
    for pack in list_packs(workspace):
        ...
        try: sections = json.loads(index_path.read_text(encoding="utf-8"))["sections"]
        ...
        for section in sections:
            ...
            results.append({
                "pack_id": pack["id"], "pack_name": pack["name"], "scope": pack["scope"],
                "version": pack["version"], "sha1": pack["sha1"], "section": section["section"],
                "excerpt": text[:500], "score": len(overlap) / len(tokens),
                "citation": f"{pack['name']} v{pack['version']}, {section['section']}",
            })
