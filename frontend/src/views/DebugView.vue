...
import { useEngagementCrumb, useTrail } from '../composables/useShell'
...
/**
 * This view sits beside the workspace shell rather than inside it, so it names
 * the engagement in the trail itself. It kept the global header *and* drew a
 * 70 px band of its own — three rows of chrome over a page of telemetry — and
 * the band's own `← Engagement` link is what the engagement crumb is now.
 */
useEngagementCrumb(() => ({ id: props.id, name: overview.value?.workspace?.name || props.id }))
useTrail(() => [{ label: 'Diagnostics' }])
...
<template>
  <div class="debug-page">
    <!-- One page header on the shared system, not a band of its own: the
         engagement and the page are in the trail above, so what is left here
         is the title, what the page holds, and its two controls. -->
